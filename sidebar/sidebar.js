var SidebarApp = (() => {
  ...
  // sidebar/sidebar-src.js
  var sidebar_src_exports = {};
  __export(sidebar_src_exports, {
    switchTab: () => switchTab
  });
  ...
  editHandlers.compositionstart = editHandlers.compositionupdate = (view) => {
    if (!view.composing) {
      ...
      if (state.selection instanceof TextSelection && (state.storedMarks || !$pos.textOffset && $pos.parentOffset && $pos.nodeBefore.marks.some((m) => m.type.spec.inclusive === false) || chrome2 && windows && selectionBeforeUneditable(view))) {
  ...
  function dropCursor(options = {}) {
    return new Plugin({
      view(editorView2) {
        return new DropCursorView(editorView2, options);
      }
    });
  }
  var DropCursorView = class {
    constructor(editorView2, options) {
      var _a;
      this.editorView = editorView2;
      this.cursorPos = null;
      this.element = null;
      this.timeout = -1;
      ...
      this.handlers = ["dragover", "dragend", "drop", "dragleave"].map((name) => {
        ...
        editorView2.dom.addEventListener(name, handler);
        return { name, handler };
      });
    }
    destroy() {
    ...
    update(editorView2, prevState) {
      if (this.cursorPos != null && prevState.doc != editorView2.state.doc) {
        if (this.cursorPos > editorView2.state.doc.content.size)
          this.setCursor(null);
        else
          this.updateOverlay();
      }
  ...
  function menuBar(options) {
    return new Plugin({
      view(editorView2) {
        return new MenuBarView(editorView2, options);
      }
    });
  }
  var MenuBarView = class {
    constructor(editorView2, options) {
      ...
      this.root = editorView2.root;
      this.wrapper = crelt("div", { class: prefix + "-wrapper" });
      this.menu = this.wrapper.appendChild(crelt("div", { class: prefix, role: "toolbar" }));
      this.menu.className = prefix;
      this.menu.ariaControlsElements = [editorView2.dom];
      if (editorView2.dom.parentNode)
        editorView2.dom.parentNode.replaceChild(this.wrapper, editorView2.dom);
      if (options.position === "after") {
        this.wrapper.insertBefore(editorView2.dom, this.wrapper.firstChild);
      } else {
        this.wrapper.appendChild(editorView2.dom);
      }
  ...
  async function saveToVault(isAutosave = false) {
    ...
    const folder = document.getElementById("fm-folder").value.trim() || settings.defaultFolder || "Clippings";
...
${stripped}`;
  ...
  function initFolderCombobox() {
    ...
    function showList(items) {
      ...
      items.forEach((folder, i2) => {
        const li = document.createElement("li");
        li.className = "folder-option";
        li.setAttribute("role", "option");
        li.setAttribute("id", `folder-opt-${i2}`);
    ...
    input.addEventListener("input", () => {
      ...
      folderDebounceTimer = setTimeout(async () => {
        ...
        const matches2 = query ? folders.filter((f) => f.toLowerCase().includes(query)) : folders;
        showList(matches2.slice(0, 20));
      }, 250);
    });
  ...
  async function syncFromVault() {
    if (!currentFilePath) {
      showToast("Save the note first before syncing.", "error");
      return;
    }
  ...
  function showConflictBanner(conflictPath) {
    ...
    banner.querySelector(".conflict-body").textContent = `The vault version differs from your editor. Your editor copy was saved as "${conflictPath.split("/").pop()}". Choose which version to keep in the editor.`;
    banner.hidden = false;
  }
  ...
  function renderTags() {
    ...
    tags.forEach((tag, i2) => {
      ...
      chip.innerHTML = `${escapeHTML(tag)}<button class="tag-chip-remove" data-index="${i2}" title="Remove tag" aria-label="Remove ${escapeHTML(tag)}">\xD7</button>`;
  ...
  async function openNoteInEditor(filePath) {
    ...
    try {
      ...
      if (fmMatch) {
        ...
        document.getElementById("note-title").value = titleMatch ? titleMatch[1].replace(/^['"]|['"]$/g, "").trim() : filenameStem;
        const inlineMatch = fm.match(/^tags:\s*\[([^\]]*)\]/m);
        const blockMatch = fm.match(/^tags:\s*\r?\n((?:[ \t]+-[^\r\n]*\r?\n?)+)/m);
        if (inlineMatch) {
          tags = inlineMatch[1].split(",").map((t) => t.trim().replace(/^["']|["']$/g, "")).filter(Boolean);
        } else if (blockMatch) {
          tags = blockMatch[1].split(/\r?\n/).map((line) => line.replace(/^\s*-\s*/, "").trim()).filter(Boolean);
        }
        renderTags();
      } else {
  ...
  async function testVaultConnection() {
    ...
    try {
      ...
      if (result?.error) throw new Error(result.error);
      const name = result?.data?.manifest?.name || result?.data?.service || "Connected";
  ...
  function markdownToHTML(md) {
    return md.replace(/^---\n([\s\S]*?)\n---\n?/, (_, fm) => `<pre><code class="frontmatter">${escapeHTML(fm)}</code></pre>
`).replace(/^#{6}\s(.+)$/gm, "<h6>$1</h6>").replace(/^#{5}\s(.+)$/gm, "<h5>$1</h5>").replace(/^#{4}\s(.+)$/gm, "<h4>$1</h4>").replace(/^#{3}\s(.+)$/gm, "<h3>$1</h3>").replace(/^#{2}\s(.+)$/gm, "<h2>$1</h2>").replace(/^#{1}\s(.+)$/gm, "<h1>$1</h1>").replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>").replace(/`([^`]+)`/g, "<code>$1</code>").replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>').replace(/^[-*]\s(.+)$/gm, "<li>$1</li>").replace(/(<li>[\s\S]*?<\/li>\n?)+/g, (m) => `<ul>${m}</ul>`).replace(/^>\s(.+)$/gm, "<blockquote><p>$1</p></blockquote>").replace(/^---$/gm, "<hr>").replace(/\n\n+/g, "</p><p>").replace(/^(?!<[a-z])(.+)$/gm, (m) => m ? `<p>${m}</p>` : "");
  }
...
`;
      ...
      case "ol":
        return "\n" + Array.from(node.children).map((li, i2) => `${i2 + 1}. ${nodeToMd(li).trim()}`).join("\n") + "\n";
      case "li":
        return kids();
      case "script":
      case "style":
  ...
  return __toCommonJS(sidebar_src_exports);
})();
